...
New spec class added 2026-05-09: **MortgageShockSpec** — static horizontal bar chart with hardcoded payment-shock data (BoC SAN 2025-21). No transforms or date range buttons. Currently used on the Housing deep-dive Mortgage Renewal Shock section.
...
Add two new `PageSpec` entries to `build.py` with placeholder content, nav bar integration, and output files `trade.html` and `demographics.html`. Follow the same pattern as the existing six placeholder deep-dive pages. See `analyses/deep-dive-design-2026-05-09.md` for the planned chart list for these two pages. Automated — no user input needed.

### 2. Fix Beveridge curve chart; build HP-filter potential GDP

- The Beveridge curve currently lives in `analyses/beveridge_curve_canada.py` and is embedded as an iframe in the Labour deep-dive placeholder. Fix and integrate it properly as a real chart in `labour.html`.
- Build an HP-filter-based potential GDP series alongside the BoC Valet output gap (`INDINF_OUTGAPMPR_Q`) — this was listed as an open item under "Output gap implementation" in the judgment items above.

### 3. Flesh out all deep-dive pages (automated)

All 8 deep-dive pages get real charts per `analyses/deep-dive-design-2026-05-09.md`:
- **Policy** — already 7 live charts; may need additions per design doc
- **Housing** — already 5 live charts; may need additions per design doc
- **Inflation, GDP, Labour, Financial, Trade, Demographics** — currently placeholder; implement real charts per design doc

Automated — no user input needed. Work from `analyses/deep-dive-design-2026-05-09.md` as the spec.
