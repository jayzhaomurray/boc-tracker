...
### Current PAGES definition (9 charts, 1 section blurb)

```
PageSpec("Bank of Canada Tracker", sections={0: "inflation"}, ...)
  ── INFLATION (heading + blurb) ──
  CpiSpec                  — CPI (Headline SA + NSA + Food + Energy + Goods + Services, Y/Y default, 10Y default)
...
All charts default to 10Y window. Only the inflation section currently has a heading + blurb above it; the other three sections are not yet wired to chart indices and not yet generated by `analyze.py`.
...
5. CAD pass-through rule of thumb (~10% depreciation → 1–2% goods CPI) — needs source [External Conditions]
6. Petrocurrency relationship characterization — when has it broken down historically? [External Conditions]
7. WCS-WTI normal differential — tracked spread or footnote? [External Conditions]
...
- Inflation is the only section currently wired through `analyze.py`; adding more is a matter of writing a `compute_*_values` + `format_*_values` pair and registering them in `analyze.py`'s `SECTIONS` dict, then adding the chart-index mapping to `PageSpec.sections`.
- The current `data/blurbs.json` was hand-generated in conversation by Opus 4.7 (the same model `analyze.py` calls). Once `ANTHROPIC_API_KEY` is set, the blurb regenerates automatically on each run.
...
- [ ] **Framework verification for non-Inflation sections** — Policy Rates, Labour, External Conditions still need their thresholds and signals verified
...
The pipeline is built but the current `data/blurbs.json` was hand-generated by Opus in the building session. To validate that the script works as designed, set the env var and run `python analyze.py`. The blurb should be regenerated automatically and look comparable to the hand-generated version.
...
### 2. Continue framework verification for the other three sections

Same process used for Inflation:
- Policy Rates next: highest-priority unresolved item is the neutral rate range (~2.5–3% for Canada). Needs a current BoC r* citation.
- Labour: NAIRU range (~5–5.5%) and the productivity assumption (~1%) embedded in the 3% wage-growth threshold.
- External Conditions: CAD pass-through rule of thumb, petrocurrency relationship, WCS spread treatment.

After each section is verified, write its `compute_*_values` + `format_*_values` in `analyze.py`, register it in `SECTIONS`, add `{<chart_idx>: <section_id>}` to `PageSpec.sections`, and run end-to-end.

### 3. Add AI-generated-content disclaimer

Short, unobtrusive note in the About section: "Section blurbs are AI-generated from public data using a fixed analytical framework. Not investment advice."
