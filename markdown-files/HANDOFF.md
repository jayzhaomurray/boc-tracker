...
### 1. OIS / market-implied rate path for the Monetary Policy section

Currently market expectations are read off the 2Y yield (Canada 2Y minus Overnight spread). Per the framework's own caveat, this is *directional only* — the 2Y embeds a term premium so magnitudes are biased. Practitioner-grade reads use OIS (overnight index swaps) or event-contract data.

The user has explicitly flagged this as the **next priority** after the balance sheet (which is now done). Implementation requires:

- Identify the right BoC Valet series for CORRA OIS rates at common tenors (1m, 3m, 6m, 12m). Likely candidates exist; need to verify by querying the Valet series list. May need to compute implied probabilities ourselves from forward rates.
- Alternatively / additionally: scrape Kalshi event contracts for individual BoC meeting decision probabilities. Kalshi has a public API; reliability and contract liquidity vary.
- Add as a new chart in the Monetary Policy section (likely below the Balance Sheet) OR as toggleable lines on the 2-Year Yields chart, depending on what the data looks like.
- Update `analysis_framework.md` Monetary Policy section to reference OIS-derived signals; remove the "2Y is a rough proxy" caveat once OIS is the primary read.

### 2. Set `ANTHROPIC_API_KEY` and confirm `analyze.py` works end-to-end
...
### 3. Framework verification + blurb generation for Monetary Policy
...
### 4. Continue framework verification for Labour Market and Financial Conditions
...
### 5. Inflation expectations data for the Inflation section
...
### 6. Add AI-generated-content disclaimer
...
### 7. Eventually: deep-dive Monetary Policy page
