...
├── housing.html            ← Housing deep-dive (5 live charts); generated by build.py
...
**Policy deep-dive landed 2026-05-09 (commit `52ab9d4`)** — `policy.html` is now a real page with 7 live charts (GoC Yield Curve, 2Y–10Y Spread, Real Rates, CORRA vs Target, BoC Assets, BoC Liabilities, Peer Central Banks).

**Housing deep-dive landed 2026-05-09 (commit `4279fac`)** — `housing.html` is now a real page with 5 live charts (CREA Sales Activity, 5Y Mortgage Rate vs 5Y GoC, Units Under Construction, Resale Activity by CMA, Housing Starts vs CMHC Target). Note: `units_under_construction` (v52300170) is Tier 2 pending — vector ID inferred from magnitude, verify with StatsCan before citing.

Recommended implementation order going forward: **GDP+Labour → Inflation → Financial → Trade+Demographics**.

For the full chart list and dependencies, see `analyses/deep-dive-design-2026-05-09.md`. Remaining deep-dives:
- **GDP & Labour:** share the `hours_worked_monthly` fetch; GDP capacity utilization and output gap data paths to decide
- **Inflation, Financial, Trade, Demographics:** outlined in design doc
