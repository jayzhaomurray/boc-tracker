...
- **Tightness: job vacancy rate (12M MA) and the V/U ratio.** Vacancy data is monthly NSA (Table 14-10-0371; no SA series published) so the analytical read uses a 12-month moving average to denoise the seasonal Sep-peak/Dec-trough pattern. The V/U ratio — vacancies per unemployed — is the BoC's preferred composite tightness indicator: V/U > 1 = more vacancies than unemployed (tight market, employer competition for workers); V/U < 1 = more unemployed than vacancies (slack, worker competition for jobs). The 2022 post-COVID peak was around V/U = 1; 2017–2019 pre-pandemic readings were closer to V/U = 0.4–0.6. Read alongside unemployment rate: a rising V/U at falling unemployment is more confidently tight than either alone.
...
- **V/U ≈ 1** = balanced (one vacancy per unemployed person); **V/U > 1** = tight (employer competition for workers, the 2022 post-COVID episode); **V/U < 0.5** = clear slack (the 2017–2019 pre-pandemic baseline was 0.4–0.6).
...
> **VERIFICATION STATUS: verified end-to-end (May 2026).** Audited against CMHC Housing Market Outlook 2026 + Spring 2026 Supply Report + 2023 Housing Shortages report; BoC Financial Stability Reports 2024 + 2025; BoC Staff Analytical Note 2025-21 (mortgage renewals); BoC SAN 2023-19 (interest rate impact on payments); BoC's Real Estate Market Definitions page; CMHC Spring 2026 permits-to-starts lag analysis; StatsCan NHPI methodology and historical releases. Material corrections applied to the original hypothesis-grade draft.
...
- **Housing starts level and Y/Y.** The headline construction read. Heavily noisy month-to-month — a 3-month moving average filters most of the noise. Long-run average since 1977 is ~194k SAAR. Recent 10-year averages have been higher (~245k) but **CMHC has explicitly flagged that even 245k is well below the affordability-restoring pace**: the 2023 CMHC Housing Shortages report estimated Canada needs ~430-500k starts/year through 2030 to close the 4.8M-unit affordability gap.
- **New Housing Price Index vs CREA MLS HPI — cycle-turn divergence as analytical signal.** NHPI tracks contractor-reported prices for newly built single-detached, semi-detached, and row homes across 27 CMAs; **it excludes condo apartments** (a major share of recent supply), custom-built homes, and resale. CREA MLS HPI is hedonic on all MLS transactions (resale-dominant) and is the index the BoC's own affordability indicator is anchored to. **When the two indices diverge directionally, NHPI is typically the lagging series** — builders are slow to cut asking prices on the downside. Both NHPI and CREA Y/Y are tracked; directional agreement or divergence at the current reading is an explicit compute output.
- **Residential building permits.** Leading indicator with **type-dependent lag**: per CMHC's Spring 2026 analysis, single-detached permits lead starts by 2-10 months, but **multi-unit / high-rise permits lead by 9-15+ months** (Calgary high-rise ~17 months; Toronto ~11 months; Montreal ~16 months). Since high-rise/rental is now the dominant Canadian housing type, the *blended* lag is closer to 9-15 months than the older 6-12 month rule. Y/Y direction signals whether the pipeline is filling or emptying — but with a longer lead-time than commonly cited.
- **Housing affordability (BoC INDINF_AFFORD_Q).** Ratio of mortgage payment to household income; higher = less affordable. The BoC and CMHC frame this as a primary transmission channel from monetary policy to household financial stress. The ratio is tracked against its 5-year range (to identify where affordability sits in the post-COVID cycle) and historical range (2000-present, ~0.28-0.55). Pre-2015 era norm was roughly 0.30-0.40; the 2022-23 peak stress episode pushed the ratio above 0.50. Improving affordability (ratio falling) signals that lower rates and/or stagnant prices are reducing the mortgage-payment burden relative to income.
...
- **Housing starts ~200-250k SAAR** = consistent with recent historical average; CMHC labels this "maintenance pace" — prevents the affordability gap from widening rapidly but does NOT close it. **Affordability-restoring pace per CMHC: 430-500k/year.**
...
- **Affordability ratio ~0.28-0.40** = pre-2015 era historical norm. **Ratio >0.40** = elevated relative to long-run average. **Ratio >0.50** = peak-stress territory (2022-23 episode). **Ratio falling Y/Y** = affordability improving, consistent with lower policy rates passing through to mortgage costs.
...
> **VERIFICATION STATUS: verified end-to-end (May 2026).** Audited against BoC MPR January 2025 In Focus on CAD drivers; SAN 2025-2 on monetary policy and the dollar; BoC Discussion Paper 2015 on FX pass-through to CPI; Alberta Central research on the petrocurrency relationship and TMX impact; CAPP on WCS-WTI differential. Compute and format functions in `analyze.py`; blurb in `data/blurbs.json` under `external`.
