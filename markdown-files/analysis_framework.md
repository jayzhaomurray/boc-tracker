...
**Data:** unemployment_rate, lfs_wages_all (Y/Y), lfs_wages_permanent (Y/Y), seph_earnings (Y/Y), lfs_micro (Y/Y, BoC composition-adjusted), cpi_services (Y/Y), cpi_all_items (for real-wages calculation).
...
- **What this framework doesn't track but BoC explicitly does.** Employment-to-working-age-population ratio, involuntary part-time rate, average hours worked, job vacancy rate, newcomer and youth unemployment composition, and unit labour costs (which combines wage growth with productivity for a more direct inflation-pressure read). Each is part of how BoC reads the labour market. The framework's omission means the labour blurb's view is *partial* — it covers wages and a single unemployment measure, not the full labour-market dashboard. Flag any conclusion accordingly.
