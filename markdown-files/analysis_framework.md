...
**Question:** What is happening in Canadian housing — construction (supply), prices (new home values), the forward-looking pipeline (permits), and the transmission to monetary policy?

**Data:** housing_starts (monthly SAAR, units in thousands), new_housing_price_index (NHPI, monthly NSA, Dec 2016 = 100), residential_permits (monthly SA value; series begins Jan 2018).
...
- **New Housing Price Index direction.** Y/Y in NHPI tracks contractor-reported prices for newly built single-detached, semi-detached, and row homes across 27 CMAs. **NHPI excludes condo apartments** (a major share of recent supply), custom-built homes, and resale. CREA's MLS HPI is the resale-dominant comparison (technically hedonic on all MLS transactions but resale dominates). The two indices typically diverge at cycle turns: NHPI tends to lag CREA on the downside because builders are slow to cut asking prices. **The BoC's own affordability indicator uses MLS resale prices, not NHPI** — NHPI is one input, not the primary one.
...
- **Other tracked-but-unmodeled channels.** Resale activity (CREA MLS units sold, sales-to-new-listings ratio); housing affordability index (BoC's own indicator using MLS prices, mortgage rates, NIAE income data); 5-year fixed mortgage rate spreads vs. 5-year GoC bond yields (FSR tracks as lender risk indicator); mortgage debt-service ratio (BoC FSR 2024: "over one-third of new mortgages had MDSR > 25%, double the 2019 share"); units under construction (CMHC pipeline metric distinct from starts); regional breakdowns (Toronto, Vancouver account for ~60% of CMHC's national supply gap).
...
**What to surface:** Lead with what housing activity is *doing* — picking up, slowing, holding. Anchor on housing starts (Y/Y or 3-month average). Reference NHPI Y/Y for the new-home price side, and explicitly note that NHPI is partial: excludes condo apartments and resale. Note permits direction as forward-looking, with the longer 9-15 month lag for the current market mix. Connect to monetary policy stance — restrictive slows housing, accommodative speeds it up — and surface the mortgage renewal wave as a quantified BoC-flagged transmission channel when the cycle position warrants. The framework's view is partial: no resale activity (CREA), no mortgage rate spreads, no MDSR, no regional breakdowns, no under-construction count, no affordability index. Flag accordingly when conclusions need a fuller picture.
