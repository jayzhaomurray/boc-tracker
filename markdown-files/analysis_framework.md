...
- **Demand-side decomposition (the contributions chart).** Components plus the **statistical discrepancy** (which the framework does not currently track) sum to the total Q/Q AR. Which is contributing most, which is dragging? Common patterns: consumption-led (typical expansion), inventory-driven (often noise; recent swings have been -4.2pp), export-led (Canada is small open economy; US demand dominates), investment-led (most rate-sensitive component), government drag/boost (fiscal stance).
